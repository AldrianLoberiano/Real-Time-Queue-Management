...
describe('QueueContext', () => {
  ...
  it('joinQueue increments counter', () => {
    const r = renderHook(() => useQueue(), { wrapper });
    result = r.result;
    act(() => { vi.advanceTimersByTime(10100); act(() => { result.current.joinQueue('Alice'); }); });
    act(() => { vi.advanceTimersByTime(10100); act(() => { result.current.joinQueue('Bob'); }); });
  ...
  it('getPosition returns correct position for waiting items', () => {
    ...
    act(() => { vi.advanceTimersByTime(10100); act(() => { result.current.joinQueue('Alice'); }); });
    act(() => { vi.advanceTimersByTime(10100); act(() => { result.current.joinQueue('Bob'); }); });
    act(() => { vi.advanceTimersByTime(10100); act(() => { result.current.joinQueue('Charlie'); }); });
